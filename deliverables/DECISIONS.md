...
### Decision: Fix the EVAL-011/012 tool-routing loop via tool-description rewording + a mirrored per-tool retry cap

- **Phase:** Post-Phase-9 (a Phase 8 finding, deliberately left undecided at the time; the user asked to address it once Phase 9 was accepted)
- **Context:** Phase 8 found the agent could exhaust its global 10-tool-call budget on EVAL-011/012 instead of answering. Re-reading the actual code (not re-guessing) found the precise, avoidable root cause: `search_work_items`'s own docstring said "Use this for any question about GitHub issues, **work items**, or their status/owner" — directly inviting the model to route EVAL-011's "the temporary Atlas **work item**" question there instead of to `search_company_knowledge`, where the document actually lives. The per-tool retry guard (`SEARCH_RETRY_LIMIT=3`) only covered `search_company_knowledge`, so a wrong-tool loop there was unbounded until the global ceiling ended the run.
- **Options considered:** (1) reword the colliding tool descriptions only; (2) mirror the existing per-tool retry cap onto `search_work_items` only; (3) both — description fix to reduce how often misrouting happens at all, retry cap as a backstop for whenever it still does.
- **Coding-agent contribution:** Implemented option 3. Verified live, with lean Groq usage per the user's explicit request (a handful of calls, not a harness re-run): (a) one direct reproduction attempt of a separate, still-unexplained Phase 8 failure sub-pattern (0–1 tool calls before `structured is None`) — it did *not* reproduce on this attempt, and a live UI check later that session incidentally reproduced it on an unrelated, normally-passing case (EVAL-002) moments after that same question had succeeded via identical code — strong evidence it's the already-documented Groq provider flakiness (Phase 6 finding), not a distinct bug, and unrelated to this fix; (b) one direct call reproducing EVAL-011's exact question against a sandboxed temp document (mirroring `evaluation/run.py`'s own EVAL-011 sandbox) — confirmed the agent now calls `search_company_knowledge` first, never touches `search_work_items`, and answers correctly citing `DOC-ATLAS-TEMP` in 3 tool calls, down from the original 6–8-call loop; (c) one regression spot-check on EVAL-002 — still answers correctly.
- **Evidence reviewed:** `tools/knowledge.py`'s exact docstring text (the collision, found by re-reading, not assumed); `ToolCallLimitMiddleware`'s actual signature (confirmed `tool_name=None` means "one shared counter across all tools" — the existing global cap — while `tool_name="X"` creates an independent per-tool counter, the mechanism to mirror); the two live verification calls above.
- **Decision and owner:** Option 3 — the user asked directly to fix this, after an `EnterPlanMode`/`ExitPlanMode` plan review; testing scope (lean Groq usage, not a full harness re-run) was set by the user's explicit amendment to that plan.
- **Consequences or follow-up:** `get_support_case`/`list_project_status`/`open_source` remain uncapped individually — no evidence they loop the way the two search tools did; still bounded by the existing global cap. The unrelated 0–1-tool-call Groq-flakiness pattern remains open, now with stronger evidence of its cause (see the EVALUATION_REPORT.md Failure Analysis entry) but no attempted fix — a provider-side issue, not something this codebase can reliably prevent.
- **Status:** Accepted

### Decision: Progressive per-step UI feedback instead of reducing round-trip time

- **Phase:** Post-Phase-9 (the latency half of the same user request)
- **Context:** Phase 8 measured ~18-26s average latency for a clean agent answer — slow for a chat UI backed only by a static "Thinking..." spinner. The user was asked directly how far to go: also try to cut actual round-trip time (smaller tool-call budget or a faster model — both touch evaluation-sensitive behavior and would need re-running Phase 8's cases to confirm no regressions), or fix perceived latency only.
- **Options considered:** (1) reduce `MAX_TOOL_CALLS` and/or evaluate a faster Groq model; (2) add live, per-tool-call progress feedback in the UI without changing any model/agent behavior.
- **Coding-agent contribution:** Implemented option 2, the user's chosen scope. `answer_with_agent` (`agent/__init__.py`) gained an optional `on_step: Callable[[str], None] | None = None` parameter — when set, it switches its internal `agent.invoke(...)` for `agent.stream(..., stream_mode="values")`, calling `on_step` with a short human-readable message (`_describe_tool_call`, a small name→message map plus a source-id-aware case for `open_source`) for each real tool call observed as the stream advances, then assembles the exact same final `Answer` once the stream completes. `app.py` wraps the call in `st.status("Thinking...", expanded=True)`, passing `status.write` as `on_step`, and collapses/marks it complete or error once the answer resolves. Verified live in a real browser: the status panel shows "Searching company knowledge..." then "Reading DOC-ATLAS-403..." as they happen, and the final answer renders normally afterward, zero exceptions.
- **Evidence reviewed:** confirmed `create_agent`'s compiled graph is a `langgraph.graph.state.CompiledStateGraph` and supports `.stream(stream_mode="values")`, yielding the full cumulative state after each step (so the last yielded chunk is equivalent to what `.invoke()` would have returned — no duplicate logic needed for assembling the final `Answer`).
- **Decision and owner:** Option 2 — the user chose this scope directly via `AskUserQuestion`, explicitly ruling out the round-trip-reduction option to avoid re-opening evaluation-sensitive behavior right after Phase 8/9 were accepted.
- **Consequences or follow-up:** Every caller that doesn't pass `on_step` (`api.py`, `service.py` call sites, the evaluation harness) is completely unaffected — same `agent.invoke()` path, same return value, confirmed via a no-Groq-cost import/signature smoke check. Actual round-trip latency is unchanged; only perceived wait time improves. Revisit the round-trip-reduction option separately (with its own re-evaluation pass) if perceived-latency relief turns out not to be enough.
- **Status:** Accepted

### Decision: Structure the main UI into persistently-boxed sections; embed the role indicator inside the chat input itself

- **Phase:** Post-Phase-9 (user-driven usability/visibility polish, iterated live over several rounds)
- **Context:** The user found the app's layout looked unstructured — content areas looked like broken/empty space until populated, the employee selector's border was invisible except when focused, and they wanted a persistent role indicator that wouldn't scroll out of view while browsing a long conversation.
- **Options considered for the boxed layout:** wrap each logical section (title, employee selector, pending-actions queue, conversation) in `st.container(key=...)` with custom CSS giving it a light-gray rounded background and explicit empty-state placeholder text, vs. leaving Streamlit's default unstyled flow. Chosen: the boxed version, per the user's explicit design request.
- **Options considered for the role indicator's persistence, in order tried:** (1) a separate boxed section with `position: sticky` — implemented, then found broken: Streamlit wraps every element in a wrapper div barely taller than the element itself, and CSS sticky only travels within its *immediate parent's* height, so there was never enough slack for it to actually stick; reverted rather than ship broken behavior. (2) Move it to the sidebar — robust but not positioned near the prompt as asked. (3) `position: fixed` with JS to match the chat input's bounds — real engineering effort and fragility, not attempted. (4) **The user's own suggestion**, adopted: render it as a CSS overlay directly on `[data-testid="stChatInput"]` (which already has `position: relative` and is natively pinned to the bottom by Streamlit's own internal `stBottom` mechanism), so it inherits "always visible" for free with no custom positioning logic at all.
- **Coding-agent contribution:** Diagnosed the sticky failure precisely (measured the wrapper's height via Playwright before concluding it was structurally blocked, not just mistuned) rather than guessing at CSS fixes. Implemented option 4 via a `::before` pseudo-element with `content: "{role}"`, colors matching `st.badge()`'s actual palette (measured live per role: engineering/blue, customer_success/green, finance/orange, people_operations/violet — not guessed), positioned with `right: 56px` (tuned after the user reported it initially overlapped the send button) and `padding-right` added to the textarea so typed text never renders under it. Also fixed the selectbox's invisible-when-idle border (its default border color exactly matched its own background — confirmed via computed-style inspection, not assumed) by giving it the same visible neutral border color the Pending-actions expander already uses, scoped to `:not(:focus-within)` so the existing focus-state indicator is untouched.
- **Evidence reviewed:** Playwright DOM/computed-style inspection at every step (wrapper heights, ancestor `overflow` properties, exact badge RGBA values, chat-input internal structure) — every fix here was measured, not guessed. Live screenshots confirmed each iteration before moving to the next.
- **Decision and owner:** The boxed-layout structure and section boundaries were the user's explicit request; the technical approach for each (CSS-only overlay over sticky/fixed/JS) was proposed by the coding agent and adopted directly once the user suggested the chat-input-overlay angle, which the agent confirmed was both feasible and more robust than any option it had proposed itself.
- **Consequences or follow-up:** The badge overlay targets `[data-testid="stChatInput"]`/`[data-testid="stChatInputTextArea"]` — stable `data-testid` attributes, not the emotion-hashed class names used elsewhere in Streamlit's DOM, so it should survive routine Streamlit version bumps better than a class-based selector would, though this hasn't been tested against an actual version change. Purely cosmetic — no evaluation-relevant behavior touched, not re-run through Phase 8's harness.
- **Status:** Accepted

### Decision: Fix the Dockerfile to include `material/`, found while rebuilding for session handoff

- **Phase:** Post-Phase-9 (found during this session's final rebuild, not during Phase 9 itself)
- **Context:** `material/northstar-logo-256.png` was added and committed in a later, separate step after Phase 9's Dockerfile was already written — the Dockerfile's `COPY` list was never updated to match. This went unnoticed through several rounds of UI iteration because all of it was verified against the local `uv run streamlit run app.py` dev server, never the container, until this session's final rebuild-for-handoff step.
- **Options considered:** N/A — a one-line omission with one correct fix, not a design tradeoff.
- **Coding-agent contribution:** Found by actually rebuilding and checking the container live (rather than assuming Phase 9's packaging still matched the current file set) — the container crashed with `MediaFileStorageError: Error opening 'material/northstar-logo-256.png'` the moment the page tried to render `st.image(...)`. Added `COPY material/ material/` to the Dockerfile; rebuilt; reconfirmed the logo, the boxed layout, and the chat-input badge overlay all render correctly through the actual container.
- **Evidence reviewed:** The container's own crash traceback (`ui-1` logs); a follow-up live Playwright check against the rebuilt container with zero exceptions.
- **Decision and owner:** The fix itself — decided by the coding agent, a clear omission with no alternative worth considering. Worth noting as a general lesson for this project going forward: a passing *local* UI check is not evidence the Docker image is current — only actually rebuilding and checking the container is.
- **Consequences or follow-up:** None outstanding. Future asset additions (images, fonts, other static files under a new top-level directory) should be checked against the `Dockerfile`'s explicit `COPY` list, not assumed to be picked up automatically.
- **Status:** Accepted

### Decision: Pull the NiceGUI + k3s packaging experiment forward, ahead of Phase 10, and isolate it entirely in `k3s-exp/`

- **Phase:** NiceGUI + k3s packaging experiment (originally planned to start only after Phase 10 — see the earlier "Defer a NiceGUI + k3s/Traefik packaging experiment" entry above)
- **Context:** The user asked to skip Phase 10's documentation deliverables for now and start this experiment immediately instead. Separately, the original plan (rewrite `app.py`'s Streamlit UI as NiceGUI in place) was reconsidered mid-planning: `AGENTS.md` treats alternative interfaces as optional extensions, never a replacement of the core Streamlit/Groq path, and an in-place rewrite would have made Streamlit and NiceGUI mutually exclusive rather than additive.
- **Options considered:** (1) rewrite `app.py`/`pages/evaluation.py` in place, as originally planned; (2) build the NiceGUI interface entirely under a new `k3s-exp/` folder — its own `app.py` (own bare `FastAPI()` instance, never touching `company_assistant.api:app`), own `Dockerfile`, own `docker-compose.yml`, own `k8s/` manifests — importing the shared `company_assistant` package (already interface-agnostic) rather than forking it, and touching nothing at the repo root beyond one new dependency (`nicegui`).
- **Coding-agent contribution:** Proposed option 1 first (matching the originally-recorded plan); the user explicitly corrected this mid-review ("it should not become a local rewrite... but a separate version... everything related could go... to the 'k3s-exp' folder"), then confirmed via direct questions that the shared business-logic package should stay imported (not forked) and dependencies should stay in the root `pyproject.toml` (not a second lockfile) — narrowing exactly how isolated "isolated" should be.
- **Evidence reviewed:** `AGENTS.md`'s own "alternative interfaces are optional extensions" rule; confirmed `company_assistant`'s existing interface-agnosticism (no Streamlit/FastAPI imports in the business-logic package) directly in the source before relying on it.
- **Decision and owner:** Option 2 — the user's explicit, direct correction, not a coding-agent judgment call.
- **Consequences or follow-up:** Root `app.py`, `pages/evaluation.py`, `Dockerfile`, and `docker-compose.yml` are untouched and continue to work exactly as Phase 9 left them. `k3s-exp/` has no shared Dockerfile/compose/k8s config with the root app — any future change to one does not need to be mirrored in the other.
- **Status:** Accepted

### Decision: Single combined container for `k3s-exp`, not a split ui/api topology like the root app's

- **Phase:** NiceGUI + k3s packaging experiment
- **Context:** The root app's Phase 9 packaging already established a precedent — one image, two `docker-compose` services (`api`, `ui`). The user explicitly said splitting into two containers for `k3s-exp` too would be acceptable "if that's cleaner to layout," reopening the question rather than assuming the collapse-to-one-process framing from planning was mandatory.
- **Options considered:** (1) two containers (heavy `api` + thin `ui` calling it over HTTP), mirroring the root app's own pattern; (2) one combined container (NiceGUI mounted directly onto its own FastAPI app, in-process).
- **Coding-agent contribution:** Weighed the split against the user's real cluster constraints (a single SUSE Leap 16 VM, ~3.5GB free RAM, ~15GB free disk, measured directly): splitting doesn't reduce total memory, since the heavy ML dependencies (torch, chromadb, sentence-transformers, langchain) load once regardless of which process owns them, and two separate Python/uvicorn processes add their own duplicate baseline overhead. Splitting would also have broken the progressive per-tool-call chat status (a single blocking HTTP call from `ui` to `api`) unless a new streaming endpoint were built — real extra scope for no resource benefit.
- **Evidence reviewed:** The VM's actual free-RAM/disk figures as reported by the user; NiceGUI's own `ui.run_with()` implementation detail (locates the running `uvicorn.Server` via `gc.get_objects()`, which only works single-worker) as an independent, non-resource-related reason 1 replica/1 process was already required.
- **Decision and owner:** Option 2 — presented as a recommendation with explicit reasoning; the user confirmed via direct question rather than the coding agent deciding unilaterally, given the user had explicitly reopened the question.
- **Consequences or follow-up:** `k3s-exp/k8s/deployment.yaml` is a single `Deployment`, `replicas: 1`, `strategy: Recreate` — scaling this beyond 1 replica would break chat/action/feedback state (kept in the one process's memory) and is not supported.
- **Status:** Accepted

### Decision: Chat calls `answer_with_agent()` via direct Python import, not `httpx` — reverses the originally recorded plan

- **Phase:** NiceGUI + k3s packaging experiment
- **Context:** The deferred plan (recorded before this session, in the earlier "Defer a NiceGUI + k3s/Traefik packaging experiment" entry) explicitly said to use `httpx` inside NiceGUI handlers calling the app's own `/ask` endpoint, for a "clean client/API boundary." That plan predates the post-Phase-9 progressive per-tool-call chat status feature (`on_step` on `answer_with_agent`), which only works as an in-process Python callback — a network round-trip through `/ask` would lose it entirely unless a new streaming (SSE) endpoint were built.
- **Options considered:** (1) keep the httpx boundary as originally planned, accept losing progressive status (a single "Thinking..." spinner instead); (2) build a new SSE endpoint on the API side specifically to carry progressive status over HTTP; (3) call `answer_with_agent()` directly, in-process, since `k3s-exp/app.py` is genuinely one process now — no real isolation is bought by a loopback HTTP call to itself.
- **Coding-agent contribution:** Recommended option 3 directly (not put to the user as an open question) — with `k3s-exp/app.py` defining its own bare FastAPI instance (not reusing `company_assistant.api:app`), there is no separate "api" process for httpx to legitimately reach across; the boundary would be purely ceremonial. `k3s-exp/app.py` similarly uses direct imports for actions/feedback/resync, matching how the root Streamlit `app.py` already does the same for the same reason.
- **Evidence reviewed:** `agent/__init__.py`'s `answer_with_agent()` signature and its `on_step` mechanism (confirmed it only works as an in-process callback via `agent.stream()`, not exposed over `/ask`).
- **Decision and owner:** Option 3 — coding-agent recommendation, unchallenged by the user.
- **Consequences or follow-up:** `company_assistant.api:app` (the plain FastAPI service) remains completely untouched and is not used by `k3s-exp/app.py` at all — it stays the stable HTTP contract for the evaluation harness or any future external client, just not for this UI.
- **Status:** Accepted

### Decision: Cross-build for `linux/amd64`; deploy via artifacts + a manual how-to, not agent-executed

- **Phase:** NiceGUI + k3s packaging experiment
- **Context:** The user's dev machine (where this session ran) is `arm64` (Apple Silicon); the k3s VM is `x86_64` — confirmed via `uname -m` on both ends of the reasoning, not assumed. Separately, this session had no `~/.kube/config` and no SSH config entry for the VM.
- **Options considered:** Build mechanism — plain `docker build` (wrong architecture) vs. `docker buildx build --platform linux/amd64 --load` (correct). Deploy access — (1) the user shares SSH/kubeconfig access so the coding agent can run the build/transfer/apply/verify steps itself; (2) the coding agent produces the Dockerfile/manifests/README, and the user runs the actual deploy themselves.
- **Coding-agent contribution:** Verified the cross-built image actually works before handing it over, rather than trusting `docker images`' size output — which reported a misleadingly small size (a known `buildx --load` + build-attestation quirk) for a build that first *looked* incomplete. Confirmed by actually running the image under emulation (`docker run --platform linux/amd64`) and checking key imports plus a real `/health` response.
- **Evidence reviewed:** `uname -m` on the dev machine; `docker history`'s misleading 0B layer sizes on the cross-built image, followed by a real emulated run proving the image was in fact complete and functional.
- **Decision and owner:** Cross-build mechanism — coding-agent decision, not really a choice (the only correct option). Deploy access model — the user's explicit choice via direct question: option 2 (artifacts + manual how-to).
- **Consequences or follow-up:** `k3s-exp/k8s/README.md` is written assuming the user runs every command themselves, including image transfer (`scp`) and `k3s ctr images import`. No cluster/VM credentials of any kind were shared with or handled by the coding agent.
- **Status:** Accepted

### Decision: Two runtime gaps found and fixed after the first live k8s deploy — evaluation-dashboard PVC, and Hugging Face Hub offline mode

- **Phase:** NiceGUI + k3s packaging experiment (found after the user's first successful deploy to their cluster, not during local verification)
- **Context:** (1) The user reported `/evaluation` saying "No results yet" on the live pod. Root cause: `data/generated/evaluation_results.json` (the Phase 8 harness's output) is git-ignored and never baked into the image — the exact same gap the root app already hit and fixed in Phase 9 (`docker compose cp`, see the "Package as two docker-compose services..." decision above), just not carried over to the k8s manifests when they were first written. (2) The user reported an unauthenticated-Hugging-Face-Hub rate-limit warning in the pod's logs, despite the embedding model being baked into the image at build time specifically to avoid a runtime network dependency.
- **Options considered:** For (1): document as a known limitation vs. add a one-time populate step. For (2): ignore the warning (harmless) vs. set `HF_TOKEN` (suppresses the warning but keeps the network dependency and adds an unneeded credential) vs. force `HF_HUB_OFFLINE=1`/`TRANSFORMERS_OFFLINE=1` (removes the network dependency entirely, matching what the bake-in step was already supposed to achieve).
- **Coding-agent contribution:** For (1): added `k8s/README.md` step 8a (`kubectl cp` the local results file into the pod once) and reproduced the exact bug locally first (fresh volume → confirmed "No results yet" → applied the fix → confirmed real data) before writing the fix up, rather than assuming the documented command would work. Traced why an earlier "verified through the container" claim had missed this: a leftover bare local dev-server process was still bound to the same port and silently answered test traffic instead of the container for anything depending on the filesystem difference between the two — see the coding agent's own memory note on this for future sessions. For (2): chose the offline-mode env vars over `HF_TOKEN`, set only *after* the model-download `RUN` step in the Dockerfile (must come after, or the download itself breaks), then rebuilt and verified both the native and the `linux/amd64` image start cleanly with the warning gone and `/health` still responding.
- **Evidence reviewed:** Direct container logs before and after each fix; a full local reproduce-then-fix cycle for (1); direct log inspection plus a fresh emulated-image boot for (2). The user separately confirmed both fixes live on the actual cluster (results now visible, warning gone, and a scale-to-0-and-back cycle preserving all state).
- **Decision and owner:** Both fixes — coding-agent diagnosis and fix, confirmed working by the user on the real cluster.
- **Consequences or follow-up:** Step 8a in `k8s/README.md` needs re-running only if the `northstar-generated-data` PVC is ever recreated (survives ordinary pod restarts and scale-down/up, confirmed by the user). None outstanding.
- **Status:** Accepted
