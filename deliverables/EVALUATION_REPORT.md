...
- **Tool-routing failures: fixed post-Phase-9.** EVAL-011's question ("the temporary Atlas **work item**") reads like a GitHub issue and steered the agent toward `search_work_items` instead of `search_company_knowledge`, where the actual document lives — traced to `search_work_items`'s own docstring literally listing "work items" as a synonym for GitHub issues, directly inviting the collision. Fixed two ways: (1) reworded both tools' docstrings in `tools/knowledge.py` so `search_work_items` is explicitly GitHub-issues-only and `search_company_knowledge`'s description explicitly covers a "work item" described in a document/Slack/email; (2) mirrored the existing `search_company_knowledge` per-tool retry cap (`ToolCallLimitMiddleware(tool_name=..., run_limit=SEARCH_RETRY_LIMIT, exit_behavior="continue")`) onto `search_work_items`, so a wrong-tool loop on *either* tool is capped rather than unbounded until the global ceiling. Re-verified live against the exact previously-failing case (sandboxed temp doc, EVAL-011's question): the agent now calls `search_company_knowledge` first, never touches `search_work_items`, and answers correctly citing `DOC-ATLAS-TEMP` in 3 tool calls. Spot-checked EVAL-002 for regression — still answers correctly. See `DECISIONS.md`.
...
- **Usability or feedback failures:** the feedback control was verified functionally in Phase 7 (both ratings persist with exactly the intended fields); no numeric usefulness target was set (thresholds table, above) since real usage is too new for a meaningful rate. **One real usability gap found and fixed post-Phase-9:** per-turn latency (~18-26s average for a clean agent answer, per Phase 8's measurements) is slow for a chat UI with only a static spinner. Fixed with progressive feedback, not by making the model faster (that would touch evaluation-sensitive behavior, deliberately out of scope here per the user's own call): `answer_with_agent` gained an optional `on_step` callback, and `app.py` now uses `st.status(..., expanded=True)` to show live per-tool-call progress ("Searching company knowledge...", "Reading DOC-ATLAS-403...") instead of one opaque "Thinking..." spinner — verified live in a real browser. Every other caller (`api.py`, the evaluation harness) is unaffected since `on_step` defaults to `None`.
...
- ~~**Tool-routing failures on ambiguous phrasing**~~ — fixed post-Phase-9 (tool-description rewording + a per-tool retry cap on `search_work_items` mirroring the existing `search_company_knowledge` one). Re-verified live against the exact failing case; see the Failure Analysis entry above and `DECISIONS.md`. The *other* sub-pattern from Phase 8 (some rows stopped after 0–1 tool calls with the same generic message) remains open and is now better understood to be **unrelated** to tool routing: live-reproducing it, the exact same question (EVAL-002) that had just answered correctly via identical code failed this way moments later in a fresh call — strong evidence this is the already-documented Groq provider flakiness (Phase 6's "provider reliability finding"), not a distinct bug, and not something the tool-routing fix addresses or needs to.
